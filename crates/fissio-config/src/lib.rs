...
impl NodeBuilder {
    ...
    /// Enables observability with default settings.
    pub fn observe(mut self) -> Self {
        self.observe = Some(fissio_monitor::ObserveConfig::new());
        self
    }

    /// Sets custom observability configuration.
    pub fn observe_config(mut self, config: fissio_monitor::ObserveConfig) -> Self {
